...
export async function retrieveRelevantInformation(queryVector: any): Promise<string[]> {
    // Get the collection
    ...
    if (results.documents && results.documents[0] && results.distances && results.distances[0]) {
        results.documents[0].forEach((doc, i) => {
            if (doc && results.distances?.[0]?.[i] !== undefined && results.distances[0][i] < relevanceThreshold) {
