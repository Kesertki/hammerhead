...
export const ChatHistory = forwardRef<HTMLDivElement, ChatHistoryProps>(function ChatHistory(
    { simplifiedChat, generatingResult, className },
    ref
) {
    const renderChatItems = useMemo(() => {
        if (simplifiedChat.length > 0 && simplifiedChat.at(-1)!.type !== 'model' && generatingResult)
    ...
    return (
        <div ref={ref} className={classNames('appChatHistory', className)}>
            {renderChatItems.map((item, index) => {
                ...
                if (item.type === 'user') return <UserMessage key={item.id} message={item} />;
