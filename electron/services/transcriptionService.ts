...
export class TranscriptionService {
	...
	static async transcribeWithWhisper(
		...
		try {
			...
			const result = await this.runChildProcess(
				'docker',
				dockerArgs,
				60000
			); // 60 second timeout
			...
			if (result.exitCode !== 0) {
				throw new Error(
					`Docker Whisper failed with exit code ${result.exitCode}: ${result.stderr}`
				);
			}

			// Read the generated JSON file
			const transcriptionData =
				await this.readTranscriptionJSON(jsonOutputPath);
			...
			try {
				...
				console.warn(
					'Failed to clean up transcription JSON file:',
					error
				);
			}
	...
	static async checkWhisperAvailability(): Promise<boolean> {
		try {
			...
			const dockerResult = await this.runChildProcess(
				'docker',
				['--version'],
				5000
			);
			...
			const imageResult = await this.runChildProcess(
				'docker',
				['images', '-q', 'whisper'],
				5000
			);
	 ...
	 */
	private static async readTranscriptionJSON(
		jsonPath: string
	): Promise<TranscriptionResult> {
		try {
	 ...
	 */
	static async transcribeWithFallback(
		audioFilePath: string
	): Promise<TranscriptionResult> {
