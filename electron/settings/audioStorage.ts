...
class AudioStorage {
    ...
    async initialize(): Promise<void> {
        ...
        try {
            ...
            console.log('Audio storage initialized at:', this.audioDirectoryPath);
        } catch (error) {
    ...
    async saveAudioBlob(blob: Buffer, duration: number, mimeType: string = 'audio/webm'): Promise<AudioMetadata> {
    ...
    async listAudioFiles(): Promise<AudioMetadata[]> {
        ...
        try {
            ...
            return audioFiles.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        } catch (error) {
    ...
    private isAudioFile(filename: string): boolean {
        const audioExtensions = ['.webm', '.m4a', '.mp4', '.ogg', '.wav', '.mp3'];
...
export const saveAudioBlob = (blob: Buffer, duration: number, mimeType?: string) =>
